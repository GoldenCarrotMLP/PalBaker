// palbaker-ui/components/mod-manager/mod-card-expanded/edit-variant-modal.tsx
"use client"
...
export function EditVariantModal({
  basePal, modName, variant, variantIndex, altermaticMetadata, traitsDb,
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      try {
        const res = await ModManagerAPI.altermaticSidecar(basePal, modName, selectedSkeletonSource)
    ...
    fetchSlots()
  }, [selectedSkeletonSource, modName, basePal])

  const filteredTraits = useMemo(() => {
    const q = traitSearch.trim().toLowerCase()
    if (!q) return []
    return Object.entries(traitsDb).filter(([display, id]) => display.toLowerCase().includes(q) || id.toLowerCase().includes(q)).filter(([, id]) => !reqTraits.includes(id) && !prefTraits.includes(id)).slice(0, 5)
  ...
  const updateMorphValue = (target: string, key: string, value: any) => {
    setMorphs((prev) => prev.map((m) => m.Target === target ? { ...m, [key]: value } : m))
  }

  const handleOpenInBlender = async () => {
    onNotify(`Opening ${selectedSkeletonSource} in Blender...`, "info", "Blender Dispatch")
    try {
      await ModManagerAPI.altermaticOpenBlend(basePal, modName, selectedSkeletonSource, altermaticMetadata?.category)
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await ModManagerAPI.altermaticDelete(basePal, modName, variantIndex)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="bg-background border border-border rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <select value={selectedSkeletonSource} onChange={(e) => setSelectedSkeletonSource(e.target.value)} className="flex h-9 w-full rounded border border-input bg-transparent px-3 py-1 text-sm shadow-sm appearance-none pr-8 cursor-pointer focus-visible:outline-none">
                  ...
                  {selectedSkeletonSource !== "base" && (
                    <button onClick={handleOpenInBlender} className="px-3 rounded border border-primary/40 bg-primary/5 text-primary text-xs font-semibold hover:bg-primary/10 transition-colors cursor-pointer">Blender</button>
                  )}
                </div>
              </div>
            </div>
          ...
          {!variant.is_base && (
            <div className="border border-border rounded-lg p-4 bg-muted/10 flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-1.5 min-h-[32px] border border-border rounded p-2 bg-background/50">
                {reqTraits.length === 0 && prefTraits.length === 0 && <span className="text-xs text-muted-foreground italic">No conditions added. This variant will spawn by default.</span>}
                {reqTraits.map((id) => {
                  ...
                  return (
                    <span key={id} className="flex items-center gap-1 bg-green-950 border border-green-800 text-green-300 text-[10px] font-bold px-2 py-0.5 rounded">
                      Req: {label}
                      <button onClick={() => setReqTraits((p) => p.filter((x) => x !== id))} className="hover:text-white shrink-0 cursor-pointer"><X className="size-3" /></button>
                    </span>
                  )
                })}
                {prefTraits.map((id) => {
                  ...
                  return (
                    <span key={id} className="flex items-center gap-1 bg-purple-950 border border-purple-800 text-purple-300 text-[10px] font-bold px-2 py-0.5 rounded">
                      Pref: {label}
                      <button onClick={() => setPrefTraits((p) => p.filter((x) => x !== id))} className="hover:text-white shrink-0 cursor-pointer"><X className="size-3" /></button>
                    </span>
                  )
                })}
              </div>
              <div className="flex flex-col gap-1.5 relative">
                <input type="text" placeholder="Fuzzy Search Passive Traits (e.g. Swift, Legend...)" value={traitSearch} onChange={(e) => setTraitSearch(e.target.value)} className="flex h-9 w-full rounded border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary" />
                  ...
                  <div className="absolute top-10 left-0 right-0 border border-border bg-background shadow-lg rounded-md z-40 flex flex-col overflow-hidden">
                    {filteredTraits.map(([display, id]) => (
                      <div key={id} className="flex items-center justify-between px-3 py-2 border-b border-border/50 hover:bg-muted/40 transition-colors">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => { setReqTraits((p) => [...p, id]); setTraitSearch("") }} className="px-2 py-1 border border-green-700/50 bg-green-950/20 text-[10px] text-green-400 rounded font-semibold hover:bg-green-950/50 cursor-pointer">+ Req</button>
                          <button onClick={() => { setPrefTraits((p) => [...p, id]); setTraitSearch("") }} className="px-2 py-1 border border-purple-700/50 bg-purple-950/20 text-[10px] text-purple-400 rounded font-semibold hover:bg-purple-950/50 cursor-pointer">+ Pref</button>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div key={morphName} className="border border-border/50 rounded-md p-3 bg-background/40 flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="relative w-[150px]">
                          <select value={type} onChange={(e) => updateMorphType(morphName, e.target.value as any)} className="flex h-7 w-full rounded border border-input bg-transparent px-2.5 text-xs appearance-none pr-6 cursor-pointer">
        ...
        <div className="flex items-center justify-between mt-2 border-t border-border pt-4">
          <button onClick={handleDelete} className="text-xs text-status-error hover:underline">Delete Variant</button>
          <div className="flex items-center gap-3">
