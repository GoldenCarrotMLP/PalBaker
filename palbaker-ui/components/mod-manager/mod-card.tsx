...
export function ModCard({
  ...
  const handleReplacerChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    onUpdateMod(mod.id || mod.name, { active_vanilla_replacer: val })
    try {
  ...
  const displaySubtitle = mod.is_variant ? `Variant of ${mod.base_pal}` : (showMapped ? mod.name : null)
  const hasPhysicalVariants = !mod.is_variant && mod.physical_variants && mod.physical_variants.length > 0
    ...
    <>
      <div
        ...
        className={cn(
          ...
          isSubVariant ? "bg-card/85 border-border/70" : "bg-card border-border",
          expanded ? "border-border/90 shadow-sm" : "",
          mod.source_modified && "border-l-2 border-l-status-warning",
          !mod.has_fmodel && !hasPhysicalVariants && "border-l-2 border-l-status-error",
          hasPhysicalVariants && "border-l-2 border-l-status-success",
          mod.has_ue && !mod.source_modified && "border-l-2 border-l-status-success",
        )}
      >
        <div className={cn("flex items-center gap-3 px-4", isSubVariant ? "py-2.5" : "py-3.5")}>
          <ModCardIcon mod={mod} />
          ...
          {/* Collapsible Sub-Directory Toggle for Parent Card */}
          {!mod.is_variant && variantsCount > 0 && (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation()
              ...
              className={cn(
                "flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-all cursor-pointer select-none shrink-0",
                variantsExpanded
              ...
              title={variantsExpanded ? "Collapse variants directory" : "Expand variants directory"}
            >
              ...
              <ChevronDown className={cn("size-3.5 transition-transform duration-200", variantsExpanded && "rotate-180")} />
            </button>
          )}

          {/* Vanilla Replacer Selector */}
          {!mod.is_variant && hasPhysicalVariants && (
            <div className="flex flex-col gap-0.5 max-w-[150px] shrink-0 mr-1">
          ...
          {/* Card Expand Details Arrow */}
          <button
            onClick={onToggle}
            disabled={!mod.has_fmodel && !hasPhysicalVariants}
            className="p-1.5 rounded text-muted-foreground hover:text-foreground hover:bg-accent transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
          ...
          <button
            ...
            className={cn(
              "px-3.5 py-1.5 rounded text-xs font-bold tracking-wider uppercase transition-colors whitespace-nowrap shrink-0 cursor-pointer",
              primary.actionClass,
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 w-64 bg-popover border border-border rounded-md shadow-xl py-1 text-sm">
                  ...
                  {[
                    { label: "True Recursive Cook & Pack", action: "recursive_cook", disabled: !mod.has_ue },
                    { label: "Create / Rebuild .blend file", action: "create_blend", disabled: !mod.has_psk },
                    { 
                      label: mod.preserve_materials === false ? "Push to Unreal (Overwrite)" : "Push to Unreal (Preserve)", 
                      action: "push",       
                      disabled: !hasPhysicalVariants && (!mod.has_fmodel || !mod.has_blend) 
                    },
                    { label: hasPhysicalVariants ? "Recursive Cook (Compile only)" : "Cook (Compile only)", action: "cook_only", disabled: !hasPhysicalVariants && !mod.has_ue },
                    { label: hasPhysicalVariants ? "Recursive Pack (Package only)" : "Pack (Package only)", action: "pack_only", disabled: !hasPhysicalVariants && !mod.has_ue },
                    { label: hasPhysicalVariants ? "Recursive Cook & Pack" : "Cook & Pack (Skip Import)", action: "cook", disabled: !hasPhysicalVariants && !mod.has_ue },
                    { label: hasPhysicalVariants ? "Recursive Push, Cook & Pack" : "Push & Cook & Pack", action: "full", disabled: !hasPhysicalVariants && (!mod.has_fmodel || !mod.has_blend) },
