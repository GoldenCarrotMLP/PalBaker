# Creating Dynamic Pal Variants

Creating a Dynamic Pal variant workspace is fully automated inside PalBaker. You can create material-only reskins (extremely lightweight) or clone complete skeletons to build custom 3D geometries.

## Step-by-Step Variant Creation

1. Expand the Mod Card of an extracted Pal.
2. Locate the **Dynamic Pal Variants** panel on the right and toggle **ENABLE** to active.
...
3. Confirming this will prune the variant from your `_dynamic_pals.json` manifest and permanently delete its custom `.blend` model from your disk.